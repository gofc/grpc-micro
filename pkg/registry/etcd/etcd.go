...
func decode(ds []byte) *registry.Service {
	...
	if err := json.Unmarshal(ds, &s); err != nil {
		logger.Error("failed decode registry service", zap.Error(err))
	}
	return s
}
...
func (e *etcdRegistry) registerNode(s *registry.Service, node *registry.Node, opts ...registry.RegisterOption) error {
	...
	if leaseID > 0 {
		logger.Debugf("Renewing existing lease for %s %d", s.Name, leaseID)
		if _, err := e.client.KeepAliveOnce(context.TODO(), leaseID); err != nil {
			if err != rpctypes.ErrLeaseNotFound {
				return err
			}

			logger.Debugf("Lease not found for %s %d", s.Name, leaseID)
	...
	if ok && v == h && !leaseNotFound {
		logger.Debugf("Service %s node %s unchanged skipping registration", s.Name, node.Id)
		return nil
	}
	...
	logger.Debugf("Registering %s id %s with lease %v and leaseID %v and ttl %v", service.Name, node.Id, lgr, lgr.ID, options.TTL)
...
func (e *etcdRegistry) Deregister(s *registry.Service) error {
	...
	for _, node := range s.Nodes {
		...
		logger.Debugf("deregistering %s id %s", s.Name, node.Id)
