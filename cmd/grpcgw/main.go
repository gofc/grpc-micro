...
import (
	"context"
	"flag"
	"github.com/gofc/grpc-micro/internal/grpcgw"
...
func init() {
	...
	if err := configor.Load(grpcgw.Conf, filePath); err != nil {
		panic(err)
	}
}
...
func main() {
	...
	gatewayServer := grpcgw.NewGatewayServer()

	pb.RegisterUserServiceHandlerServer(grpcServer, gatewayServer)
	...
	if err = server.Register(code, ts.Addr().String(), conf.Registry.Address); err != nil {
